...
contract('Test DebtEngine Diaspore', function (accounts) {
  ...
  async function toTokens (amount, oracle = { address: ethers.constants.AddressZero }, oracleData = '') {
    if (oracle.address === ethers.constants.AddressZero) {
      return amount;
    }
  ...
  async function withFee (amount, oracle = { address: ethers.constants.AddressZero }, oracleData = '') {
    return bn(amount).add(await toFee(amount, oracle, oracleData));
  }

  async function toFee (amount, oracle = { address: ethers.constants.AddressZero }, oracleData = '') {
  ...
  it('Should create different ids create2 and create3', async function () {
    const expireTime = (await getNow()).add(bn(2000));
  ...
  it('Funds should follow the debt', async function () {
    ...
    const id = await getId(debtEngine.create(
      ...
      accounts[0],
      ethers.constants.AddressZero,
      await testModel.encodeData(laonAmount, (await getNow()).add(bn(2000)), 0, (await getNow()).add(bn(2000))),
    ));
    ...
    expect(await rcn.balanceOf(payer)).to.equal(payAmount.sub(await withFee(laonAmount)));
    expect(await debtEngine.getStatus(id)).to.equal(STATUS_PAID);
    expect(await testModel.getPaid(id)).to.equal(laonAmount);
    ...
    expect(await rcn.balanceOf(accounts[6])).to.equal(laonAmount);
  });
  it('Calling pay, payTokens, payBatch or payBatchTokens should get the same rate', async function () {
    const id1 = await getId(debtEngine.create(
      ...
      accounts[0],
      oracle.address,
      await testModel.encodeData(toWei('900000'), (await getNow()).add(bn(2000)), 0, (await getNow()).add(bn(2000))),
    ));
    ...
    await debtEngine.payToken(id1, toWei(1), ethers.constants.AddressZero, data);
    await debtEngine.payTokenBatch([id3], [toWei(1)], ethers.constants.AddressZero, oracle.address, data);

    const paid1 = await testModel.getPaid(id1);
    expect(paid1).to.equal(await testModel.getPaid(id3));

    await debtEngine.pay(id2, paid1, ethers.constants.AddressZero, data);
    await debtEngine.payBatch([id4], [paid1], ethers.constants.AddressZero, oracle.address, data);

    expect(paid1).to.equal(await testModel.getPaid(id4));
    expect(paid1).to.equal(await testModel.getPaid(id2));
  });
  ...
  it('Should not be possible to brute-forze an infinite loop', async function () {
    const id = await getId(debtEngine.create(
      ...
      accounts[2],
      ethers.constants.AddressZero,
      await testModel.encodeData(3000, (await getNow()).add(bn(2000)), 0, (await getNow()).add(bn(2000))),
    ));
    ...
    expect(await rcn.balanceOf(accounts[0])).to.equal(0);
    expect(await debtEngine.getStatus(id)).to.equal(STATUS_ONGOING);
    expect(await testModel.getPaid(id)).to.equal('50');
    ...
    expect(await debtEngine.getStatus(id)).to.equal(STATUS_ONGOING);
    expect(await testModel.getPaid(id)).to.equal('150');
  });
  ...
  describe('Function setFee', function () {
    it('Should set the set fee', async function () {
      ...
      expect(await debtEngine.fee()).to.equal(newFee);
  ...
  describe('Function create', function () {
    it('Should create a debt using create', async function () {
      ...
      const data = await testModel.encodeData(bn(1000), (await getNow()).add(bn(1000)), 0, (await getNow()).add(bn(1000)));
      ...
      expect(debt.balance).to.equal(0);
      expect(debt.fee).to.equal(0);
      assert.equal(debt.model, testModel.address);
      assert.equal(debt.creator, creator);
      assert.equal(debt.oracle, ethers.constants.AddressZero);

      assert.equal(await debtEngine.ownerOf(calcId), owner);

      expect(await debtEngine.balanceOf(accounts[1])).to.equal(prevBalAcc1.add(bn(1)), 'Account 1 should have a new asset');
    });
    it('Differents debt engine should give differents ids, create', async function () {
      ...
      const id1 = await getId(engine1.create(
        ...
        accounts[0],
        ethers.constants.AddressZero,
        await testModel.encodeData(bn(3000), (await getNow()).add(bn(2000)), 0, (await getNow()).add(bn(2000))),
      ));
  ...
  describe('Function create2', function () {
    it('Should create a debt using create2', async function () {
      ...
      const data = await testModel.encodeData(bn(3000), (await getNow()).add(bn(2000)), 0, (await getNow()).add(bn(2000)));
    ...
    it('Should create 2 debts using create2', async function () {
      ...
      await debtEngine.create2(
        ...
        ethers.constants.AddressZero,
        bn('8000001'),
        await testModel.encodeData(bn(2000), (await getNow()).add(bn(3000)), 0, (await getNow()).add(bn(3000))),
      );

      expect(await debtEngine.balanceOf(accounts[2])).to.equal(prevBalAcc2.add(bn(1)), 'Account 2 should have a new asset');
    });
    it('Should predict Ids', async function () {
      const pid1 = await debtEngine.buildId2(
        ...
        ethers.constants.AddressZero,
        bn(12000),
        await testModel.encodeData(bn(1000), (await getNow()).add(bn(2000)), 0, (await getNow()).add(bn(2000))),
      );
  ...
  describe('Function buildId2', function () {
    it('It should create diferent IDs create2 with any change', async function () {
      ...
      ids.push(await debtEngine.buildId2(
        ...
        ethers.constants.AddressZero,
        bn('1200'),
        await testModel.encodeData(bn(1000), (await getNow()).add(bn(2200)), 0, (await getNow()).add(bn(2200))),
      ));
      ...
      ids.push(await debtEngine.buildId2(
        ...
        ethers.constants.AddressZero,
        bn('1200'),
        await testModel.encodeData(bn(1001), (await getNow()).add(bn(2000)), 0, (await getNow()).add(bn(2000))),
      ));
      ...
      ids.push(await debtEngine.buildId2(
        ...
        ethers.constants.AddressZero,
        bn('1200'),
        await testModel.encodeData(bn(1000), (await getNow()).add(bn(2001)), 0, (await getNow()).add(bn(2001))),
      ));
  ...
  describe('Function pay', function () {
    it('Should create and pay a debt', async function () {
      ...
      const data = await testModel.encodeData(amount, (await getNow()).add(bn(2000)), 0, (await getNow()).add(bn(2000)));
      ...
      expect(debt.balance).to.equal(amount);

      expect(await rcn.balanceOf(payer)).to.equal(plusAmount);
      expect(await debtEngine.getStatus(id)).to.equal(STATUS_PAID);
      expect(await testModel.getPaid(id)).to.equal(amount);
    });
    it('Should pay using an Oracle', async function () {
      ...
      const data = await testModel.encodeData(totalDebt, (await getNow()).add(bn(2000)), 0, (await getNow()).add(bn(2000)));
      ...
      const debt = await debtEngine.debts(id);
      expect(debt.balance).to.equal(payAmountOracle);

      expect(await rcn.balanceOf(payer)).to.equal(0);
      expect(await debtEngine.getStatus(id)).to.equal(STATUS_ONGOING);
      expect(await testModel.getPaid(id)).to.equal(_paid);
      ...
      await debtEngine.pay(id, _paid2, ethers.constants.AddressZero, oracleData2, { from: originPayer });

      expect(await rcn.balanceOf(originPayer)).to.equal(0);
      expect(await testModel.getPaid(id)).to.equal(_paid.add(_paid2));
      ...
      expect(await rcn.balanceOf(originPayer)).to.equal(payAmountOracle3.sub(restDebtOracle3));
      expect(await testModel.getPaid(id)).to.equal(totalDebt);
      expect(await debtEngine.getStatus(id)).to.equal(2);
    });
    it('Should pay with fee', async function () {
      ...
      const data = await testModel.encodeData(loanTotalAmount, (await getNow()).add(bn(2000)), 0, (await getNow()).add(bn(2000)));
      ...
      const debt = await debtEngine.debts(id);
      expect(debt.balance).to.equal(payAmount);
      expect(await rcn.balanceOf(burner)).to.equal(prevBurnerBal.add(feeAmount));

      expect(await rcn.balanceOf(payer)).to.equal(0);
      expect(await debtEngine.getStatus(id)).to.equal(STATUS_ONGOING);
      expect(await testModel.getPaid(id)).to.equal(payAmount);
      ...
      expectEvent(
        receipt2, 'Paid',
        {
          ...
          _tokens: payTotalAmount,
        },
      );
      expectEvent(receipt2, 'ChargeBurnFee', { _id: id, _amount: feeAmount2 });

      const debt2 = await debtEngine.debts(id);
      expect(debt2.balance).to.equal(loanTotalAmount);
      expect(await rcn.balanceOf(burner)).to.equal(prevBurnerBal2.add(feeAmount2));

      expect(await rcn.balanceOf(payer)).to.equal(0);
      expect(await testModel.getPaid(id)).to.equal(loanTotalAmount);
      expect(await debtEngine.getStatus(id)).to.equal('2');
    ...
    it('Should pay using an Oracle with fee', async function () {
      ...
      const debt = await debtEngine.debts(id);
      expect(debt.balance).to.equal(payAmountTokens);
      expect(await rcn.balanceOf(burner)).to.equal(prevBurnerBal.add(feeAmount));
      ...
      const debt2 = await debtEngine.debts(id);
      expect(debt2.balance).to.equal(await toTokens(loanTotalAmount, oracle, oracleData));
      expect(await rcn.balanceOf(burner)).to.equal(prevBurnerBal2.add(feeAmount2));
    ...
    it('Pay should round in favor of the owner', async function () {
      const id = await getId(debtEngine.create(
        ...
        accounts[0],
        oracle.address,
        await testModel.encodeData(10, (await getNow()).add(bn(2000)), 0, (await getNow()).add(bn(2000))),
      ));
      ...
      await expectRevert(
        debtEngine.pay(id, 1, ethers.constants.AddressZero, data),
        'ERC20: insufficient allowance',
      );

      expect(await testModel.getPaid(id)).to.equal(0);
    });
    it('Should apply rate even when tokens is not divisible by 10', async function () {
      ...
      await debtEngine.pay(id, 1000, ethers.constants.AddressZero, data);
    ...
    it('Should apply rate with token more expensive than currency', async function () {
      ...
      await debtEngine.pay(id, toWei(1), ethers.constants.AddressZero, data);
    ...
    it('Pay should fail if paid is more than requested', async function () {
      ...
      await expectRevert(
        debtEngine.pay(id, 100, ethers.constants.AddressZero, ethers.constants.AddressZero),
        'Paid can\'t be more than requested',
      );

      expect(await testModel.getPaid(id)).to.equal(0);
      expect(await rcn.balanceOf(accounts[0])).to.equal(prevBalance);
    });
    it('Pay should fail if rate includes zero', async function () {
      ...
      await expectRevert(
        debtEngine.pay(id, 1000, ethers.constants.AddressZero, data),
        'Oracle provided invalid rate',
      );

      expect(await rcn.balanceOf(accounts[0])).to.equal(value);
      expect(await testModel.getPaid(id)).to.equal(0);
    ...
    it('Pay should fail if payer has not enought balance', async function () {
      ...
      await expectRevert(
        debtEngine.pay(id, 2000, ethers.constants.AddressZero, ethers.constants.AddressZero),
        'ERC20: insufficient allowance',
      );
    ...
    it('Should catch and recover from a pay error', async function () {
      ...
      expect(await rcn.balanceOf(accounts[0])).to.equal('100');
      expect(await debtEngine.getStatus(id)).to.equal(STATUS_ERROR);
      expect(await testModel.getPaid(id)).to.equal('50');
    ...
    it('Should catch and recover from a pay error, with an Oracle', async function () {
      ...
      // Should have failed and the status should be 4
      expect(await rcn.balanceOf(accounts[0])).to.equal('50');
      expect(await debtEngine.getStatus(id)).to.equal(STATUS_ERROR);
      expect(await testModel.getPaid(id)).to.equal('50');
  ...
  describe('Function payToken', function () {
    ...
    it('Should payToken using an Oracle', async function () {
      ...
      const originPayer = accounts[3];
      const payAmountOracle = bn('60000');
      const data = await testModel.encodeData(bn('10000'), (await getNow()).add(bn(2000)), 0, (await getNow()).add(bn(2000)));
      ...
      const receipt = await debtEngine.payToken(
        id,
        payAmountOracle,
        originPayer,
        oracleData1,
      ...
      expectEvent(
        receipt, 'Paid',
        {
          ...
          _origin: originPayer,
          _requested: bn(0),
          _requestedTokens: payAmountOracle,
          _paid: _paid,
          _tokens: payAmountOracle,
        },
      );
      ...
      expect(await rcn.balanceOf(payer2)).to.equal(expectBalance);
      expect(await testModel.getPaid(id)).to.equal('10000');
      expect(await debtEngine.getStatus(id)).to.equal(STATUS_PAID);
    });
    it('Should payToken using with fee', async function () {
      ...
      const receipt2 = await debtEngine.payToken(
        id,
        payTotalAmount,
        originPayer,
        [],
      ...
      expectEvent(
        receipt2, 'Paid',
        {
          ...
          _origin: originPayer,
          _requested: bn(0),
          _requestedTokens: payTotalAmount,
          _paid: payTotalAmount,
          _tokens: payTotalAmount,
        },
      );
    ...
    it('Should payToken using an Oracle with fee', async function () {
      ...
      expectEvent(
        receipt, 'Paid',
        {
          ...
          _origin: originPayer,
          _requested: bn(0),
          _requestedTokens: payAmountTokens,
          _paid: _paid,
          _tokens: payAmountTokens,
        },
      );
      ...
      expectEvent(
        receipt2, 'Paid',
        {
          ...
          _origin: originPayer,
          _requested: bn(0),
          _requestedTokens: payTotalAmountTokens,
          _paid: payTotalAmount,
          _tokens: payTotalAmountTokens,
        },
      );
    ...
    it('Pay tokens round in favor of the owner', async function () {
      ...
      await debtEngine.payToken(id, 1, ethers.constants.AddressZero, data);
    ...
    it('Should apply rate pay tokens even when tokens is not divisible by 10', async function () {
      ...
      await debtEngine.payToken(id, toWei(1), ethers.constants.AddressZero, data);

      expect(await rcn.balanceOf(accounts[0])).to.equal('342');
      expect(await testModel.getPaid(id)).to.equal(bn('704974378193313'));
    });
    it('Should apply rate pay tokens with token more expensive than currency', async function () {
      ...
      await debtEngine.payToken(id, toWei(2), ethers.constants.AddressZero, data);

      expect(await rcn.balanceOf(accounts[0])).to.equal(bn('1834816'));
      expect(await testModel.getPaid(id)).to.equal(bn('242587496458'));
    });
    ...
    it('Pay tokens should fail if paid is more than requested', async function () {
      ...
      await expectRevert(
        debtEngine.payToken(id, 100, ethers.constants.AddressZero, ethers.constants.AddressZero),
    ...
    it('Pay tokens should fail if payer has not enought balance', async function () {
      ...
      await expectRevert(
        debtEngine.payToken(id, 2000, ethers.constants.AddressZero, ethers.constants.AddressZero),
        'ERC20: insufficient allowance',
      );
    ...
    it('Pay tokens fail if rate includes zero', async function () {
      ...
      await expectRevert(
        debtEngine.payToken(id, 1000, ethers.constants.AddressZero, data),
        'Oracle provided invalid rate',
      );
  ...
  describe('Function payBatch', function () {
    ...
    it('Pay batch should emit pay event', async function () {
      ...
      expect(oracleEvent.args._count).to.equal('2');
      expect(oracleEvent.args._tokens).to.equal('5');
      expect(oracleEvent.args._equivalent).to.equal('10');
      ...
      paidLogs.forEach((event) => {
        ...
        case id1: {
          ...
          expect(args._requested).to.equal(2000);
          expect(args._requestedTokens).to.equal(0);
          expect(args._paid).to.equal(2000);
          expect(args._tokens).to.equal(1000);
        ...
        case id2: {
          ...
          expect(args2._requested).to.equal(1000);
          expect(args2._requestedTokens).to.equal(0);
          expect(args2._paid).to.equal(1000);
          expect(args2._tokens).to.equal('500');
    ...
    it('Pay batch multiple times multiple id should be like paying the sum', async function () {
      ...
      await debtEngine.payBatch([id1, id1, id2], [1000, 1000, 500], ethers.constants.AddressZero, ethers.constants.AddressZero, []);

      expect(await rcn.balanceOf(accounts[0])).to.equal('500');
      expect(await testModel.getPaid(id1)).to.equal(2000);
      expect(await testModel.getPaid(id2)).to.equal('500');

      const debt1 = await debtEngine.debts(id1);
      expect(debt1[1]).to.equal(2000);

      const debt2 = await debtEngine.debts(id2);
      expect(debt2[1]).to.equal('500');
    });
    it('Should create and pay debts in batch', async function () {
      ...
      ids[0] = await getId(
        debtEngine.create(
          ...
          await testModel.encodeData(3000, (await getNow()).add(bn(2000)), 0, (await getNow()).add(bn(2000))), // data
        ),
      );
      ids[1] = await getId(
        debtEngine.create(
      ...
      ids[2] = await getId(
        debtEngine.create(
          ...
          accounts[2],       // owner
          ethers.constants.AddressZero,               // oracle
          await testModel.encodeData(100, (await getNow()).add(bn(2000)), 0, (await getNow()).add(bn(2000))), // data
        ),
      );
      ...
      await debtEngine.payBatch(ids, amounts, ethers.constants.AddressZero, ethers.constants.AddressZero, []);

      expect(await rcn.balanceOf(accounts[0])).to.equal('1050');
      expect(await debtEngine.getStatus(ids[0])).to.equal(STATUS_PAID);
      expect(await testModel.getPaid(ids[0])).to.equal(3000);
    });
    it('Should pay batch using a oracle', async function () {
      const id = await getId(debtEngine.create(
        ...
        accounts[0],
        oracle.address,
        await testModel.encodeData(10000, (await getNow()).add(bn(2000)), 0, (await getNow()).add(bn(2000))),
      ));
      ...
      expect(await rcn.balanceOf(accounts[2])).to.equal(0);
      expect(await testModel.getPaid(id)).to.equal(1000);
    });
    it('Pay batch should round in favor of the owner', async function () {
      ...
      await expectRevert(
        debtEngine.payBatch([id, id], [1, 0], ethers.constants.AddressZero, oracle.address, data),
        'ERC20: insufficient allowance',
      );
      await expectRevert(
        debtEngine.payBatch([id], [1], ethers.constants.AddressZero, oracle.address, data),
        'ERC20: insufficient allowance',
      );
      await debtEngine.payBatch([id], [0], ethers.constants.AddressZero, oracle.address, data);
    ...
    it('Should apply rate pay batch with token more expensive than currency', async function () {
      ...
      await debtEngine.payBatch([id], [toWei(1)], ethers.constants.AddressZero, oracle.address, data);
    ...
    it('Pay batch should fail if one debt paid is more than requested', async function () {
      ...
      await expectRevert(
        debtEngine.payBatch([id1, id2], [1000, 1000], ethers.constants.AddressZero, ethers.constants.AddressZero, []),
        'Paid can\'t be more than requested',
      );

      expect(await testModel.getPaid(id1)).to.equal(0);
      expect(await testModel.getPaid(id2)).to.equal(0);
      expect(await rcn.balanceOf(accounts[0])).to.equal(prevBalance);
    });
    ...
    it('Should pay batch with tokens less expensive than currency', async function () {
      ...
      expect(await testModel.getPaid(id1)).to.equal(2000);
      expect(await testModel.getPaid(id2)).to.equal(1000);
    ...
    it('Should create and pay a debts using payTokens in batch', async function () {
      ...
      ids[2] = await getId(
        debtEngine.create(
          ...
          accounts[2],
          ethers.constants.AddressZero,
          await testModel.encodeData(100, (await getNow()).add(bn(2000)), 0, (await getNow()).add(bn(2000))),
        ),
      );
      ...
      await debtEngine.payTokenBatch(ids, amounts, ethers.constants.AddressZero, ethers.constants.AddressZero, []);
    ...
    it('Try use payTokenBatch to pay a debt/s with invalid id/s', async function () {
      ...
      await expectRevert(
        debtEngine.payTokenBatch(
          [ethers.constants.HashZero, web3.utils.randomHex(32)],
          [0, 1],
          accounts[0],
          ethers.constants.AddressZero,
          [],
  ...
  describe('Function payTokenBatch', function () {
    ...
    it('Pay token batch shoud not overflow the debt balance', async function () {
      ...
      await debtEngine.pay(id, 3000, ethers.constants.AddressZero, ethers.constants.AddressZero);

      const debt = await debtEngine.debts(id);
      expect(debt[1]).to.equal(3000);
      ...
      await expectRevert(
        debtEngine.payTokenBatch([id2, id], [10, value], ethers.constants.AddressZero, ethers.constants.AddressZero, []),
        'ERC20: insufficient allowance',
      );

      expect(await testModel.getPaid(id)).to.equal(3000);
      expect(await testModel.getPaid(id2)).to.equal(0);

      const ndebt = await debtEngine.debts(id);
      expect(ndebt[1]).to.equal(3000);

      await rcn.setBalance(accounts[2], 0);
      await debtEngine.withdraw(id, accounts[2], { from: accounts[2] });
      expect(await rcn.balanceOf(accounts[2])).to.equal(3000);
    });
    it('Pay tokens batch should fail if one debt paid is more than requested', async function () {
      ...
      await expectRevert(
        debtEngine.payTokenBatch([id1, id2], [1000, 1000], ethers.constants.AddressZero, ethers.constants.AddressZero, []),
        'Paid can\'t be more than requested',
      );
    ...
    it('Pay tokens batch round in favor of the owner', async function () {
      ...
      await debtEngine.payTokenBatch([id, id], [1, 1], ethers.constants.AddressZero, oracle.address, data);
    ...
    it('Should not pay the third debt because not correspond the currency and oracle.', async function () {
      ...
      expect(await rcn.balanceOf(accounts[0])).to.equal('4150');
      expect(await debtEngine.getStatus(ids[0])).to.equal(STATUS_PAID);
      expect(await testModel.getPaid(ids[0])).to.equal(3000);
    });
    it('Should apply rate pay batch tokens even when tokens is not divisible by 10', async function () {
      ...
      await debtEngine.payTokenBatch([id], [toWei(1)], ethers.constants.AddressZero, oracle.address, data);
    ...
    it('Should apply rate pay batch tokens with token more expensive than currency', async function () {
      ...
      await debtEngine.payTokenBatch([id], [toWei(2)], ethers.constants.AddressZero, oracle.address, data);
    ...
    it('Pay tokens batch multiple times multiple id should be like paying the sum', async function () {
      ...
      await debtEngine.payTokenBatch([id1, id2, id1], [1000, 500, 1000], ethers.constants.AddressZero, ethers.constants.AddressZero, []);
    ...
    it('Pay token batch should emit pay event', async function () {
      ...
      paidLogs.forEach((event) => {
        ...
        case id1: {
          const args = event.args;
          expect(args._requested).to.equal(0);
          expect(args._requestedTokens).to.equal(1000);
          expect(args._paid).to.equal(2000);
          expect(args._tokens).to.equal(1000);
          assert.equal(args._sender, accounts[0]);
          assert.equal(args._origin, accounts[4]);
        ...
        case id2: {
          const args2 = event.args;
          expect(args2._requested).to.equal(0);
          expect(args2._requestedTokens).to.equal('500');
          expect(args2._paid).to.equal(1000);
          expect(args2._tokens).to.equal('500');
          assert.equal(args2._sender, accounts[0]);
          assert.equal(args2._origin, accounts[4]);
  ...
  describe('Function withdraw', function () {
    it('Should withdraw funds from payment', async function () {
      ...
      expect(await rcn.balanceOf(beneficiary)).to.equal(amount);
      ...
      expect(await rcn.balanceOf(beneficiary)).to.equal(0);
    });
    it('Pay shoud not overflow the debt balance', async function () {
      ...
      await expectRevert(
        debtEngine.pay(id, value, ethers.constants.AddressZero, ethers.constants.AddressZero),
        'uint128 Overflow',
      );
    ...
    it('Pay token shoud not overflow the debt balance', async function () {
      ...
      await debtEngine.payToken(id, 3000, ethers.constants.AddressZero, ethers.constants.AddressZero);
      ...
      await expectRevert(
        debtEngine.payToken(id, bn('2').pow(bn('129')), ethers.constants.AddressZero, ethers.constants.AddressZero),
        'uint128 Overflow',
      );
    ...
    it('Pay batch shoud not overflow the debt balance', async function () {
      ...
      await expectRevert(
        debtEngine.payBatch([id2, id], [10, bn('2').pow(bn('129'))], ethers.constants.AddressZero, ethers.constants.AddressZero, []),
        'uint128 Overflow',
      );
    ...
    it('Should fail withdraw not authorized', async function () {
      ...
      expect(await rcn.balanceOf(accounts[3])).to.equal(0);
      expect(await rcn.balanceOf(accounts[2])).to.equal(0);
    ...
    it('Should withdraw partial payments, authorized', async function () {
      const id = await getId(debtEngine.create(
        ...
        accounts[0],
        ethers.constants.AddressZero,
        await testModel.encodeData(1000, (await getNow()).add(bn(2000)), 0, (await getNow()).add(bn(2000))),
      ));
      ...
      expect(await debtEngine.getStatus(id)).to.equal(STATUS_ONGOING);
      expect(await testModel.getPaid(id)).to.equal('100');
      ...
      expect(await rcn.balanceOf(accounts[7])).to.equal('100');
      ...
      expect(await rcn.balanceOf(accounts[7])).to.equal(0);
      ...
      expect(await rcn.balanceOf(accounts[8])).to.equal('200');
    });
  });
  describe('Function withdrawPartial', function () {
    it('Should fail to withdraw partially if sender is not authorized', async function () {
      ...
      expect(await rcn.balanceOf(accounts[1])).to.equal(0);
      expect(await rcn.balanceOf(accounts[0])).to.equal(0);

      const debt = await debtEngine.debts(id);
      expect(debt[1]).to.equal(1000);
    });
    it('Should withdraw partially if sender is authorized', async function () {
      ...
      expect(await rcn.balanceOf(accounts[2])).to.equal('600');
      expect(await rcn.balanceOf(accounts[1])).to.equal(0);
      expect(await rcn.balanceOf(accounts[0])).to.equal(0);

      const debt = await debtEngine.debts(id);
      expect(debt[1]).to.equal('400');
    });
    it('Should withdraw partially total amount', async function () {
      ...
      expect(await rcn.balanceOf(accounts[2])).to.equal(1000);
      expect(await rcn.balanceOf(accounts[0])).to.equal(0);

      const debt = await debtEngine.debts(id);
      expect(debt[1]).to.equal(0);
    });
  ...
  describe('Function withdrawBatch', function () {
    it('Should withdraw funds from multiple debts', async function () {
      ...
      const data1 = await testModel.encodeData(amount1, (await getNow()).add(bn(2000)), 0, (await getNow()).add(bn(2000)));

      const amount2 = bn('7000');
      const data2 = await testModel.encodeData(amount2, (await getNow()).add(bn(2000)), 0, (await getNow()).add(bn(2000)));
      ...
      // Withdraw funds
      await rcn.setBalance(beneficiary, 0);
      const receipt = await debtEngine.withdrawBatch(
        [id1, id2],
        beneficiary,
        { from: owner },
      );
      expectEvent(
        receipt, 'Withdrawn',
        { _id: id1, _sender: owner, _to: beneficiary, _amount: amount1 },
      );
      expectEvent(
        receipt, 'Withdrawn',
        { _id: id2, _sender: owner, _to: beneficiary, _amount: amount2 },
      );

      const debt1 = await debtEngine.debts(id1);
      expect(debt1.balance).to.equal(0);

      const debt2 = await debtEngine.debts(id2);
      expect(debt2.balance).to.equal(0);

      expect(await rcn.balanceOf(beneficiary)).to.equal('10000');
    ...
    it('Should pay using an Oracle and withdraw', async function () {
      ...
      await debtEngine.pay(id, 1000, ethers.constants.AddressZero, oracleData2, { from: accounts[3] });
      ...
      expect(await rcn.balanceOf(accounts[9])).to.equal(expectBalance);
      ...
      expect(await rcn.balanceOf(accounts[9])).to.equal(0);
    });
    it('Should fail withdraw batch if debt engine has no funds', async function () {
      ...
      await debtEngine.payBatch([id, id2], [1500, 1500], ethers.constants.AddressZero, ethers.constants.AddressZero, []);
    ...
    it('Should fail withdraw batch not authorized', async function () {
      ...
      await debtEngine.payBatch([id1, id2], [1500, 1500], ethers.constants.AddressZero, ethers.constants.AddressZero, []);
    ...
    it('Should fail withdraw batch not authorized mixed', async function () {
      ...
      expect(await rcn.balanceOf(accounts[4])).to.equal('1500');
    });
  ...
  describe('Function getFeeAmount', function () {
    ...
    it('Get fee amount with % fee', async function () {
      ...
      expect(feeAmount2).to.equal(await toFee(payAmount));
    ...
    it('Get fee amount with % fee and oracle', async function () {
      ...
      expect(feeAmount2).to.equal(await toFee(payAmount, oracle, oracleData));
