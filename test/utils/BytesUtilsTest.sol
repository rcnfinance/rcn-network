...
contract BytesUtilsTest {
    ...
    function testInvalidLengthBytes() external {
        ThrowProxy throwProxy = new ThrowProxy(address(bytesUtils));
        bytes memory testData = buildData(uint256(123).toBytes32(), address(this).toBytes32(), bytes4(keccak256("test")));

        // Reading 0 & 1 items should work
        Assert.equal(bytesUtils.pReadBytes32(testData, 0), uint256(123).toBytes32(), "Read index 0, uint256");
    ...
    function testReadOffsetPacked() external {
        ...
        Assert.equal(bytesUtils.pRead(data, 0, 1), uint8(12).toBytes32(), "Read value 0");
        Assert.equal(bytesUtils.pRead(data, 1, 20).toAddress(), address(this), "Read value 1");
        ...
        Assert.equal(bytesUtils.pRead(data, 1 + 20 + 32 + 32 + 32 + 16, 1), 1.toBytes32(), "Read value 6");
        Assert.equal(bytesUtils.pRead(data, 1 + 20 + 32 + 32 + 32 + 16 + 1, 2), uint16(5355).toBytes32(), "Read value 7");
    }
    ...
    function testDecode() external {
        ...
        Assert.equal(b, 1.toBytes32(), "Decode 6 items");
