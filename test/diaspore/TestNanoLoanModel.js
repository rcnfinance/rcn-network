...
contract('NanoLoanModel', function (accounts) {
    async function encodeData (params) {
        return model.encodeData(params[0], params[1], params[2], params[3], params[4]);
    }
    ...
    it('Test validate function', async function () {
        async function tryValidate (changeIndexs, values, message) {
            ...
            for (let i = 0; i < changeIndexs.length; i++) { params[changeIndexs[i]] = values[i]; }
    ...
    it('Test E3 27% Anual interest, 30 days', eTest(10000, 27, 40.5, 30, 10, 10075, 10, 10150, 30, 10455, 5, 10512));
    ...
    it('Test E7 27% Anual interset, 30 days', eTest(70000, 27, 40.5, 30, 10, 70525, 10, 71050, 30, 73185, 5, 73587));
