...
contract('NanoLoanEngine', function (accounts) {
    ...
    it('Should allow reference loans with their identifier', async () => {
        ...
        // create a new loan
        const loanId1Identifier = await engine.buildIdentifier(sampleOracle, accounts[1], accounts[0], sampleCurrency, web3.toWei(2),
            Helper.toInterestRate(27), Helper.toInterestRate(40), 86400, 0, 10 * 10 ** 20, '');
        const loanId1 = await createLoan(engine, sampleOracle, accounts[1], sampleCurrency, web3.toWei(2), Helper.toInterestRate(27), Helper.toInterestRate(40),
            86400, 0, 10 * 10 ** 20, accounts[0], '');
        ...
        // create one a little bit different
        const loanId2 = await createLoan(engine, 0x0, accounts[3], 0x0, web3.toWei(4), Helper.toInterestRate(17), Helper.toInterestRate(46),
            86405, 2, 11 * 10 ** 20, accounts[3], 'Test');
        const loanId2Identifier = await engine.buildIdentifier(0x0, accounts[3], accounts[3], 0x0, web3.toWei(4),
    ...
    it('Should approve a loan using it\'s identifier', async () => {
        const loanIdIdentifier = await engine.buildIdentifier(0x0, accounts[3], accounts[4], 0x0, web3.toWei(4),
            Helper.toInterestRate(17), Helper.toInterestRate(46), 86405, 2, 11 * 10 ** 20, 'Test');

        const loanId = await createLoan(engine, 0x0, accounts[3], 0x0, web3.toWei(4), Helper.toInterestRate(17), Helper.toInterestRate(46),
    ...
    it('Lend should fail if loan not approved', async () => {
        // create a new loan
        const loanId = await createLoan(engine, 0x0, accounts[1], 0x0, web3.toWei(2), Helper.toInterestRate(27), Helper.toInterestRate(40),
            86400, 0, 10 * 10 ** 20, accounts[0], '');
    ...
    it('Should handle a loan with an oracle', async () => {
        ...
        // create a new loan
        const loanId = await createLoan(engine, oracle.address, accounts[1], ethCurrency, web3.toWei(1), Helper.toInterestRate(27),
    ...
    it('Test fix error pay all', async () => {
        ...
        const loanId = await createLoan(engine, 0x0, accounts[0], 0x0, 4000, Helper.toInterestRate(27), Helper.toInterestRate(40),
            86400, 0, 10 * 10 ** 20, accounts[0], '');
    ...
    it('Should work as a ERC721 token', async () => {
        ...
        // Create 5 loans
        const loanId1 = await createLoan(engine, 0x0, accounts[0], 0x0, 4000, Helper.toInterestRate(27), Helper.toInterestRate(40),
            86400, 0, 10 * 10 ** 20, accounts[0], 'Loan 1');
        const loanId2 = await createLoan(engine, 0x0, accounts[0], 0x0, 4000, Helper.toInterestRate(27), Helper.toInterestRate(40),
            86400, 0, 10 * 10 ** 20, accounts[0], 'Loan 2');
        const loanId3 = await createLoan(engine, 0x0, accounts[0], 0x0, 4000, Helper.toInterestRate(27), Helper.toInterestRate(40),
            86400, 0, 10 * 10 ** 20, accounts[0], 'Loan 3');
        const loanId4 = await createLoan(engine, 0x0, accounts[0], 0x0, 4000, Helper.toInterestRate(27), Helper.toInterestRate(40),
    ...
    it('Should work with a cosigner', async () => {
        // Create loan
        const loanId = await createLoan(engine, 0x0, accounts[0], 0x0, web3.toWei(2), Helper.toInterestRate(27), Helper.toInterestRate(40),
            86400, 0, 10 * 10 ** 20, accounts[0], '');
    ...
    it('Should withdraw batch', async function () {
        const id1 = await createLoan(engine, 0x0, accounts[1], 0x0, web3.toWei(2), Helper.toInterestRate(27), Helper.toInterestRate(40),
            86400, 0, 10 * 10 ** 20, accounts[1], '11');
        const id2 = await createLoan(engine, 0x0, accounts[1], 0x0, web3.toWei(2), Helper.toInterestRate(27), Helper.toInterestRate(40),
            86400, 0, 10 * 10 ** 20, accounts[1], '22');
        const id3 = await createLoan(engine, 0x0, accounts[1], 0x0, web3.toWei(2), Helper.toInterestRate(27), Helper.toInterestRate(40),
    ...
    function eTest (amount, interest, punitoryInterest, duesIn, d1, v1, d2, v2, d3, v3) {
        return async () => {
            ...
            const loanId = await createLoan(engine, 0x0, accounts[1], 0x0, amount, interest, punitoryInterest,
    ...
    function eTest2 (amount, interest, punitoryInterest, duesIn, d1, v1, d2, v2, d3, v3) {
        return async () => {
            ...
            let d1Diff = Math.abs(d1PendingAmount.toNumber() - v1);
            assert.isBelow(d1Diff, 2, 'The v1 should aprox the interest rate in the d1 timestamp');
