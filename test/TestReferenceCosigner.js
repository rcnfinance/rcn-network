...
contract('Test ReferenceCosigner Diaspore', function (accounts) {
    ...
    describe('function approveLoan', async () => {
        const signer = accounts[7];

        it('Should approve a loan to a new operator', async () => {
            const borrower = accounts[1];
            const lender = accounts[2];
            ...
            assert.equal(await debtEngine.ownerOf(id), cosigner.address);
            assert.equal(await debtEngine.getApprovedAddress(id), operator);
        });

        it('Try approve a loan to a new operator without ownership', async () => {
            await Helper.tryCatchRevert(
                () => cosigner.approveLoan(
                    loanManager.address,
                    bn('6266297'),
                    accounts[3],
                    { from: accounts[3] }
        ...
        it('Try approve a loan and the liability is not claimed', async () => {
            const borrower = accounts[1];
            const lender = accounts[2];
            const amount = bn('626232');
            const expirationLoan = (bn((await Helper.getBlockTime()).toString())).plus(bn('1000'));
